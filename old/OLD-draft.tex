...
\begin{abstract}
    ...
    \textcolor{red}{In case overleaf redirects here, please actually refer to the new version of the paper submission.tex}
...
Given the set of semantic restrictions and a mined rule set, we automatically append to each rule relevant exception conditions that, at application phase, will stop the triggering of the rules when they would result in a trivial violation of the considered restrictions. 

We then compare the original and extended set of rules on semantic validity of predicted triples, to explore the trade-off derived by stopping violations trivially caused by rules. Our results show that applying exceptions at single-rule level (i.e. without considering rule-rule interaction) already allows for a substantial decrease in the number of inconsistent triples predicted, with a limited increase in computational time. Additionally, while we do not focus on optimizing benchmark predictive performance, our preliminary results suggest that standard rank-based metrics are not heavily influenced by the change in expressivity of the rule sets, posing an argument in favor of the need for integrating such metrics with other, semantic-oriented, tools.
