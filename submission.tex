...
In this section, we describe the basic terminology and concepts used in the rest of the discussion.
...
\paragraph{Applying the exceptions} When materializing $\mathcal{G}^{IR^{nm}}$, for each rule \textit{$r_i$}:
\begin{itemize}
    \item We iterate over candidates $e \in \mathcal{E}$ as a substitution of the first variable $X$.
    \item If in-graph exceptions are violated, further search for the remaining variables is halted.
...
For example, in YAGO4.5 many rules predicting gender are mined (Appendix \ref{mined_rules}). However, candidate subjects often already have an existing gender relation. Any rule that would add additional gender information would therefore either generate inconsistent facts or fail to produce a complete derivation path. With the non-monotonic extensions, such rules are stopped early, allowing the algorithm to move on to more promising rules instead.

The number of inconsistent triples from the standard rule sets, relative to the newly materialized ones, varies significantly depending on the dataset. NELL has the larger ratio in both the 10\% and 30\% conditions, while Hetionet and  YAGO4.5 maintain a low percentage of inconsistent triples overall, although   YAGO4.5 reaches a noticeable fraction in the 30\% condition. In the case of Hetionet, only 2 (resp. 4) rules trigger, which have relatively low confidence and a high number of body groundings (see Appendix \ref{mined_rules}). This limits how informative they are, as we further confirm in the following section considering the predictive performance. In the case of YAGO, while a larger number of rules are applied, the number of original inconsistencies remains contained. This suggests that the KG, which is thoroughly curated, allows to mine rules of higher semantic quality, but they can still be improved by the increase in expressivity. In contrast, NELL, that is automatically created with a low curated  ontology, results in a more noisy rule sets. 

\paragraph{Rank-based metrics.} As shown in Table \ref{tab:metrics_part1}, the influence of the non-monotonic extension on hits@k and MRR is negligible. We attribute this to the quality and precision of high-confidence rules, which likely dominate the results at lower $k$ values: since individual rules can generate multiple predictions for (s,p,?) or (?,p,o) queries, they have the most influence with the top ranks. This trend is also reflected in the sem@10 scores, which indicate that the majority of top-10 predictions are semantically correct. However, the standard setting shows a slight decline in Sem@10, suggesting that inconsistencies persist even when deriving predictions from high-confidence rules.

Regarding the low LP performance, particularly e.g. Hetionet with near-zero scores, we identify two primary contributing factors. First, the  dataset is highly coarse, containing a vast number of individuals but few classes and property types. Second, we recall that our configuration restricts the expressivity of the rule miners, i.e. AnyBURL was limited to cyclic rules and AMIE was capped at rules of length  2. While these constraints were necessary to ensure tractability rather than to maximize performance, future work should explore applying non-monotonic extensions to more expressive rule sets.
